...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1,len(word)+1):
      for c in EditModel.ALPHABET:
        # Corrupt Signal is the one before the insertion
        corruptLetters = word[i-1:i]
        # Correct Signal is the character 'c' and the following character
        correctLetters = word[i-1:i] + c
        # Corrected word inserts character c at i
        correction = "%s%s%s" % (word[1:i], c, word[i:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(1, len(word)-1):
      # Corrupt Signal is at index i + 1
      corruptLetters = word[i:i+2]
      # Reverses the Substring
      correctLetters = corruptLetters[::-1]
      # Corrected word is concatenation of correct Letters + rest of substring
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      for c in EditModel.ALPHABET:
        ...
        # Correct Letter is the one being substituted (i.e the letter being corrected)
        correctLetters = c
        # Corrupt Letter is the correct letter that is substituted out
        corruptLetters = word[i]
        # Correction is the mispelled word
...
def main():
  ...
  TRANPOSE_EDITS2 = set([Edit('etst', 'te', 'et'),Edit('tset', 'es', 'se'), Edit('tets', 'st', 'ts')])
  ...
  print "Transpose edits for 'test'"
  checkOverlap(set(editModel.transposeEdits('test')), TRANPOSE_EDITS2)
